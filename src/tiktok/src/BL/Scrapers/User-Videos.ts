...
interface UserVideosResponse {
  itemList: any[]; // Здесь должен быть подходящий тип
  cursor: string;
  hasMore: boolean;
  // Другие свойства, если они существуют
}

export async function getUserVideos(userUID: string, cursor: string): Promise<UserVideosResponse> {
